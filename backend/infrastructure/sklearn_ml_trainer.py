...
class SklearnMLTrainer(MLModelTrainer):
    ...
    def __init__(self, use_fast_model: bool = False, n_jobs: int = -1):
    ...
    def train_models(self, data: pd.DataFrame, variables: List[str]) -> Dict:
        ...
        logger.info(
            f"All models trained in {total_duration:.1f}s (finished at {end_time.strftime('%H:%M:%S')})"
        )

        return models

    def _train_single_model(
        self, data: pd.DataFrame, variables: List[str], target_var: str
    ):
        """Train a single model for a target variable."""
        model_start = datetime.now()
        logger.info(
            f"[{model_start.strftime('%H:%M:%S')}] Training model for {target_var}..."
        )
        ...
        logger.info(
            f"[{model_end.strftime('%H:%M:%S')}] Completed {target_var} in {duration:.1f}s"
        )
    ...
    def predict(self, models: Dict, input_data: Dict[str, float]) -> Dict[str, float]:
        ...
        for target_var, model_info in models.items():
            ...
            feature_data = {f: input_data.get(f, 0.0) for f in features}
